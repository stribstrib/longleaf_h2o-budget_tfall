...
# 5. INPUT the dates during which you would like your tfall data summaries to begin and end
start.date <- as_date("2015-02-04") 
end.date <- as_date("2020-12-09") 
...
tfall %>% filter(smpl_date == as_datetime("2018-04-12 08:12:00"), plot %in% c(47, 48))
...
# but we need to appply a correction based on regression, since these aren't 1-to-1
...
ggplot(filter(bulk.ppt_filtered, outliers != "none"), aes(x=twr.ppt_original, y=bulk.ppt_original, color = outliers))+
  geom_point(aes(x=twr.ppt, y=bulk.ppt_mm), color = "black", alpha = 0.5, data=bulk.ppt_filtered) +
  geom_point()#+
  ylim(0,100) +
  xlim(0,50)#+
  
  ggplot(filter(bulk.ppt_filtered, twr.ppt_original > 25,  twr.ppt_original < 30), aes(x=twr.ppt_original, y=bulk.ppt_original, color = outliers))+
