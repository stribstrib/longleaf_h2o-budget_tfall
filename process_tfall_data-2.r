...
BW.merge %>% filter(is.na(smpl_start))
...
tfall <-  tfall %>%
  group_by_at(vars(c(evnt.id, trt))) %>%
  ...
  left_join(tfall, ., by = c("evnt.id", "trt")) %>% 
...
tfall.summ <- tfall %>%
  group_by_at(vars(c(evnt.id, evnt.start, evnt.end, site, trt))) %>% 
  summarise(bkt.n = n(),
...
if(force.zero == T) 
  {tfall.summ <- tfall.summ %>% 
    mutate(int = if_else(.$ppt.avg - .$tfall.avg < 0, 0, .$ppt.avg - .$tfall.avg))
  }
if(force.zero == F) 
  {
  tfall.summ <- tfall.summ %>% 
    mutate(int = (ppt.avg - tfall.avg))
  }
...
#  # Graph cumulative interception --- THIS NEEDS WORK... LOTS OF NaNs; not sure why.
