...
bkt_lvls <- c("0","1","2","3","4","5","6","7", "R1", "R2", "R3", "R4", "R5")
...
(smpl_events <- smpl_spans %>%
    ...
    mutate(RD.rx = if_else(site == "RD" & !is.na(`9`|`13`), 1, NA_real_)) %>%
    mutate(RD.ex = if_else(site == "RD" & !is.na(`36`|`37`), 1, NA_real_)) %>%
...
# ONE sample set does not work with the above merge for loops:
# Manual fix below, by row number.:
# FIRST View to ensure the manual fix will work with the row numbers
View(RD.merge)
for(i in 63:67){
  RD.merge$mrg.id[i] = 5
  RD.merge$evnt.start[i] = as.POSIXct(RD.merge$smpl_start[64])
  RD.merge$evnt.end[i] = as.POSIXct(RD.merge$smpl_date[64])
}
...
tfall.plot.summ <- tfall.p %>%
  group_by_at(vars(c(evnt.id, evnt.start, evnt.end, twr.ppt, site, trt, plot))) %>% 
...
bulk.ppt <- tfall.trt.summ %>% 
  select(evnt.id, evnt.start, evnt.end, site, trt, tfall.trt.avg) %>% 
  filter(trt == "BP") %>% 
