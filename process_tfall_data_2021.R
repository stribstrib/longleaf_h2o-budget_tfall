...
met_twr_data.fp <- "R:/ecohydrology/projects/fire_reintroduction/data/Throughfall/R/Throughfall_Longleaf_Eco/eddy.towers.ppt_gapfilled_2020-12-31.csv" 

# pre-processed (filtered & gap-filled BP data)
bulk_ppt_data.fp <- "R:/ecohydrology/projects/fire_reintroduction/data/Throughfall/R/Throughfall_Longleaf_Eco/bulk.ppt_processed_2020-12-08.csv"

# linear regression model parameters from tower-to-bulk-precip-buckets model
bulk_ppt_models.fp <- "R:/ecohydrology/projects/fire_reintroduction/data/Throughfall/R/Throughfall_Longleaf_Eco/bulk.ppt_models_2022-02-15.csv"
...
#double check missing bulk data
...
# but one set that showed up in bp processing due to filtered data point is not here, because it hasn't been filtered out here yet.
# will be taken care of in subsequent step.
...
(tfall <- tfall %>%
  ...
  select(smpl_start:tfall_mm, notes, outliers, gf = bulk.ppt_gf))
...
for(i in 1:nrow(smpl_spans_ppt)){
  ...
  smpl.span = filter(met_twr, SITE == smpl_spans_ppt$site[i], 
...
tfall <- smpl_spans_ppt %>%
  left_join(tfall, .) %>% 
  select(smpl_start:tfall_mm, twr.ppt, notes, outliers, gf)
...
# it shows up as a weird sample span since that was a span of time between last clearing of buckets & chacking of buckets,
...
#### WHERE SAMPLE EVENTS ARE THE SAME, MEASUREMENTS ACROSS THE SAME BUCKET SHOULD BE COMBINED!
...
tfall2 <- tfall2 %>% 
  group_by(evnt.id, evnt.start, evnt.end, site, plot, trt, bkt, twr.ppt) %>% 
  summarise(tot.tfall_mm = sum(tfall_mm, na.rm = T),
            n_msmts = sum(!is.na(tfall_mm)),
            tot.tfall_mm = if_else(n_msmts == 0, NA_real_, tot.tfall_mm), #without this, instances where the bucket wasn't measured at all would be summed to 0
            notes.comb = str_c(unique(notes), collapse = ', '),
            outliers.comb = str_c(unique(outliers), collapse = ', '),
            gf.comb = str_c(unique(gf), collapse = ', ')) %>% 
  select(evnt.id, evnt.start, evnt.end, site, plot, trt, bkt, tfall_mm = tot.tfall_mm, twr.ppt, n_msmts, notes = notes.comb, outliers = outliers.comb, gf = gf.comb)


## check # msmts for each bucket and sample evnt again:
...
tfall.fltrd <- tfall2 %>%
  ...
  arrange(evnt.id, trt) %>% 
  select(evnt.id, trt, avg.tfall, lo.fnc, hi.fnc) %>% 
  left_join(tfall2, ., by = c("evnt.id", "trt")) %>% 
  mutate(outliers = if_else(tfall_mm < lo.fnc | tfall_mm > hi.fnc, "tukey outlier", 
                            if_else(avg.tfall > 5 & tfall_mm < 0.1, "false 0", outliers)))#,

#check outlier filter first:
ggplot(tfall.fltrd, aes(x = avg.tfall, y = tfall_mm))+
  geom_point(data = filter(tfall.fltrd, !(outliers %in% c("both", "bucket", "tukey outlier", "tower", "false 0"))), alpha = 0.5) +
geom_point(data = filter(tfall.fltrd, outliers %in% c("both", "bucket", "tukey outlier", "tower", "false 0")), aes(color = outliers), alpha = 0.2)#+
  # coord_cartesian(xlim = c(0, 50), ylim = c(0, 100))

# performance is okay. continue with removing the outlier data
tfall.fltrd <- tfall.fltrd %>% 
  mutate(tfall_mm = if_else(trt %in% c("RX", "EX", "RE") & outliers %in% c("tukey outlier", "false 0"), NA_real_, tfall_mm)) %>% 
...
BW.int <- round(BP.mods$int[BP.mods$site == "BW"], 2)
BW.slp <- round(BP.mods$slp[BP.mods$site == "BW"], 2)

RD.int <- round(BP.mods$int[BP.mods$site == "RD"], 2)
RD.slp <- round(BP.mods$slp[BP.mods$site == "RD"], 2)  
...
tfall.plot.summ <- tfall.fltrd %>%
  ...
  ungroup() %>% 
  filter(as_date(evnt.start) >= as_date("2015-07-13")) # the date when we began measuring bulk precip at each site
...
tfall.plot.summ <- tfall.plot.summ %>% 
  ...
  mutate(gf = case_when(is.na(ppt.avg) & site == "BW" ~ paste0(twr.ppt,"*",BW.slp,"+", BW.int),
                        is.na(ppt.avg) & site == "RD" ~ paste0(twr.ppt,"*", RD.slp, "+", RD.int),
                        TRUE ~ NA_character_),
         ppt.avg = case_when(is.na(ppt.avg) & site == "BW" ~ (twr.ppt*BW.slp + BW.int),
                            is.na(ppt.avg) & site == "RD" ~ (twr.ppt*RD.slp + RD.int),
                            TRUE ~ ppt.avg),
         ppt.avg = if_else(ppt.avg >= 0, ppt.avg, 0))
  
  
# create a summary-level treatment, including only those dates of interest.
tfall.trt.summ <- tfall.fltrd %>%
  filter(as_date(evnt.start) >= as_date(start.date) &
           as_date(evnt.end) <= as_date(end.date)) %>%
...
tfall.trt.summ <- tfall.trt.summ %>% 
  ...
  mutate(gf = case_when(is.na(ppt.avg) & site == "BW" ~ paste0(twr.ppt,"*",round(BW.slp,2),"+", round(BW.int, 2)),
                        is.na(ppt.avg) & site == "RD" ~ paste0(twr.ppt,"*", round(RD.slp, 2), "+", round(RD.int, 2)),
                        TRUE ~ NA_character_),
         ppt.avg = case_when(is.na(ppt.avg) & site == "BW" ~ (twr.ppt*BW.slp + BW.int),
                            is.na(ppt.avg) & site == "RD" ~ (twr.ppt*RD.slp + RD.int),
                            TRUE ~ ppt.avg),
...
#total precip over 5 years (9/30/2015 - 9/28/2020):
# Baker: 7434.1
# Red Dirt: 7225.0
(7434 + 7225)/2 # 7330 average btwn both sites

7330/5 #average annual precip = 1466 mm
1466/12 #average monthly precip = 122

#totals for 9/30/2015 - 9/28/2020
BW.BP <- 7434
BW.RX <- 7223
BW.EX <- 6826
...
BW.RX.int <- BW.BP-BW.RX # 211 mm
BW.EX.int <- BW.BP-BW.EX # 608 mm
BW.RX.int/BW.BP #percent interception relative to total rainfall # 0.03
BW.RX/BW.BP #percent throughfall relative to total rainfall # 0.97

BW.EX.int/BW.BP #percent interception relative to total rainfall #0.08
BW.EX/BW.BP #percent throughfall relative to total rainfall # 0.92

(BW.EX.int-BW.RX.int)/BW.RX.int #percent difference between exclusion and burned sites 
#188% increase

(BW.EX.int-BW.RX.int)/yrs #79.4 mm/yr

RD.RX.int <- RD.BP-RD.RX # 78 mm
RD.EX.int <- RD.BP-RD.EX # 656 mm
RD.RX.int/RD.BP # 0.01
RD.EX.int/RD.BP # 0.09
(RD.EX.int-RD.RX.int)/RD.RX.int # 741% more interception in RE EX versus RX 

(RD.EX.int-RD.RX.int)/yrs

RD.BP/yrs #1445 mm / yr
BW.BP/yrs # 1487 mm / yr

((RD.BP/yrs)+(BW.BP/yrs))/2
